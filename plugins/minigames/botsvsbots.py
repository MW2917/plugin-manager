...
class FroshBot(SpazBot):
    ...
    throw_rate = 0.8
    throwiness = 0.2 #should it be .2 or .7?
    punchiness = 0.85
    charge_dist_max = 3.0
...
class TeamBotSet(SpazBotSet):
    ...
    def __init__(self, team: Team) -> None:
        ...
        self._bot_list_count = 5 + (activity._bots_per_team//20)
    ...
    def colorforteam(self, spaz) -> None:
        if spaz.character == 'B-9000':
            new_color = ((self.team.color[0] + spaz.node.color[0])/2,
                         (self.team.color[1] + spaz.node.color[1])/2,
                         (self.team.color[2] + spaz.node.color[2])/2,
            )
            spaz.node.highlight = self.team.color
            spaz.node.color = new_color
        elif spaz.character == 'Kronk':
            new_color = ((self.team.color[0]*2 + spaz.node.highlight[0])/3,
                         (self.team.color[1]*2 + spaz.node.highlight[1])/3,
                         (self.team.color[2]*2 + spaz.node.highlight[2])/3,
            )
            spaz.node.highlight = new_color
